---
title: "帳戶常見問題"
source_url: "https://support.evoto.ai/account-faq/"
source_type: "post"
source_id: "1346"
language: "zh-hant"
translation_group: "1346"
primary_category: "account-settings"
tags:
  - "account"
  - "account-settings"
  - "faq"
---

**本節整理了與 Evoto 帳戶相關的常見問題，包括如何註冊、管理帳戶存取，以及處理確認信與密碼重設等常見情況。**

#### 什麼是 Evoto 帳戶？

Evoto 帳戶可讓你安全地使用我們強大的照片編輯平台。帳戶啟用後，你可以：

- 在所有支援的裝置上（桌面版與 iPad）登入 Evoto App。
- 方便地管理你的點數使用情況、訂閱方案與帳單設定。
- 存取雲端預設與個人化設定，讓工作流程保持一致。
- 接收重要更新、功能公告，以及我們團隊提供的專屬支援。

---

#### 我要如何註冊帳戶？

你可以直接在 Evoto App 或官方網站上輕鬆註冊新的 Evoto 帳戶。

###### 透過 Evoto App 註冊

1. 從你裝置的 App Store 下載並安裝 Evoto App。
2. 開啟 App，並在登入畫面找到 **註冊**。
3. 輸入你的**電子郵件地址**或**手機號碼**來建立帳戶。
4. 點擊 **註冊**。
5. 輸入寄送到你註冊電子郵件或手機號碼的**驗證碼**。
6. 點擊 **登入** 完成註冊流程並進入你的帳戶。

###### 透過 Evoto 網站註冊

1. 前往 Evoto 官方網站 [evoto.ai](https://evoto.ai/)。
2. 點擊 **登入**（通常位於右上角）。
3. 你可以輸入**電子郵件地址**繼續，或選擇 **使用 Google/Facebook 繼續** 更快速地註冊。
4. 若你使用電子郵件註冊，請依照寄送到信箱中的**確認步驟**啟用帳戶。

**小提醒：**

- 請使用你平常會查看的電子郵件地址，以免錯過重要帳戶通知。
- 在某些地區，為提升帳戶安全性，註冊時可能需要進行身分驗證。
- 如需更多入門資訊，請參考我們的入門指南。

---

#### 疑難排解：沒有收到確認信？

如果你已完成註冊流程，但還沒有收到確認信，請嘗試以下步驟：

###### 1. 檢查垃圾郵件或廣告信件匣

確認信有時會被誤判到垃圾郵件或廣告信件匣。請檢查是否收到以下寄件者寄出的郵件：

- noreply@mail.evoto.ai
- noreply@mail1.evoto.ai
- noreply@mail2.evoto.ai
- noreply@mail3.evoto.ai

如果有找到，請將郵件標記為「非垃圾郵件」，確保未來通知能正常送達收件匣。

###### 2. 將我們加入白名單（安全寄件者清單）

為避免後續再次收不到信，並確保 Evoto 郵件都能送達收件匣，請將上述地址加入聯絡人或白名單/安全寄件者清單。常見信箱服務的設定方式如下：

###### Gmail

1. 前往 **Settings**（齒輪圖示）→ **See all settings** → **Filters and Blocked Addresses**。
2. 點擊 **Create a new filter**。
3. 在「From」欄位中輸入所有 Evoto noreply 郵件地址，並以逗號分隔（例如：noreply@mail.evoto.ai, noreply@mail1.evoto.ai, ...）。
4. 點擊 **Create filter**。
5. 勾選 **「永不移至垃圾郵件」**，然後完成設定。

###### Outlook / Hotmail

1. 前往 **Settings**（齒輪圖示）→ **View all Outlook settings** → **Mail** → **Junk email**。
2. 在 **Safe senders and domains** 區塊中點擊 **+ Add**，並逐一輸入上方列出的 Evoto noreply 郵件地址。
3. 點擊 **儲存**。

###### Yahoo Mail

1. 前往 **Settings**（齒輪圖示）→ **More Settings** → **Filters**。
2. 點擊 **Add new filters**。
3. 設定規則，將來自上述 Evoto 寄件者的郵件移至收件匣（例如把「From」設為包含 evoto.ai，並移動到「Inbox」）。
4. 點擊 **儲存**。

###### 還是沒收到郵件？

如果以上方法都無法解決問題，請聯絡支援團隊尋求協助：

- **電子郵件：** support@evoto.ai

---

#### 如果我忘記密碼怎麼辦？

如果你忘記 Evoto 帳戶密碼，可以直接在 App 或網站上重新設定：

1. 在登入畫面點擊 **「忘記密碼？」** 或 **「重設密碼」** 連結。
2. 輸入你註冊時使用的電子郵件地址或手機號碼。
3. 依照寄送到你信箱或手機的指示建立新密碼。

---

#### 我要如何登出 Evoto 帳戶？

若要登出 Evoto 帳戶：

1. 開啟 Evoto App。
2. 點擊右上角或側邊欄中的**帳戶頭像**（通常是藍色圓形的姓名縮寫或個人照片）。
3. 從下拉選單中選擇 **「登出」**。
